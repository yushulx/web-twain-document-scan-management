...
- **Document boundary detection**: detect the document edges on the current page, preview the auto-normalized result, manually adjust the boundary quad if needed, and replace the original page with the perspective-corrected image. Two interchangeable detection backends can be switched from the preview dialog to compare results: **Dynamsoft Capture Vision** (the DDN engine via the `DetectDocumentBoundaries_Default` preset template) and a **built-in** pure-canvas pipeline (no extra SDK or license). Both handle rotated ID cards and passports as well as full-page documents.
