...
- **Document edge detection**: run Dynamsoft Capture Vision's document detection template on the current page, preview the auto-normalized result, manually adjust the boundary quad if needed, and replace the original page with the perspective-corrected image.
